...
- 三大 API 提供的是**書目+摘要**,不是全文。全文只有開放取用(OA)版本拿得到:S2 的 `openAccessPdf`、OpenAlex 的 `openAccessPdf`(oa_url)、arXiv 直接下載。近年 CS/AI 文獻 OA 比例高(arXiv 文化),醫學/社科約四到五成,付費牆內的**就是拿不到**,誠實標 ❓ 附連結。
- 摘要品質不均:Elsevier/APA 常整個不給(null)、MDPI 給全文式長摘要、部分出版社摘要僅兩三句——「摘要沒寫」不等於「內文沒有」,這是深查升級路徑存在的原因。

## PDF 文字抽取(方程式查核/全文深查用)
